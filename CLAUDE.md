...
There is no automated test suite. Verify changes with the curl smoke flow: `POST /api/setup` (first run) → `POST /api/login` (cookie jar) → exercise `/api/entries`, `/api/tabs`, etc. AI streaming can be tested against a local mock of `POST /chat/completions` returning SSE.
