...
1. Build and start a fresh instance (NEVER port 3000 — Open WebUI owns it on this machine):
   ...
   COOKIE_INSECURE=1 AI_ALLOW_PRIVATE=1 DATA_DIR=./ui-verify-data PORT=3213 npm start   # run in background
   ```
   `COOKIE_INSECURE=1` is REQUIRED locally — the session cookie is `Secure` by default and headless Chrome won't store it over plain-HTTP localhost, so login silently fails to persist. `AI_ALLOW_PRIVATE=1` lets the SSRF guard allow a localhost mock AI provider.
...
- Initial load shows a `.boot-splash` (pulsing leaf) until `/api/me` resolves — wait for concrete view selectors, not just page load.
...
- Layout: tabs live in a left `.sidebar` (desktop: persistent, `.collapse-btn` toggles a 70px icon rail persisted in localStorage `dayleaf-sidebar`; mobile: off-canvas drawer — open via ☰ `.menu-btn`, an interactive edge swipe (sidebar follows the finger; dispatch synthetic TouchEvents from x<28 with rAF waits between moves — commit thresholds ~38% of drawer width to open, ~32% to close), close via `.sidebar-overlay` tap or leftward swipe). Tab rows are `.side-tab` (`.side-main` selects, `.side-edit` pencil and `.side-delete` trash (hover on desktop, always faintly visible on touch; delete confirms then DELETEs /api/tabs/:id),
...
- Journal: greeting header `.greet` (search toggle is the 🔍 `.search-btn` in the greet row) with `.stat-pill`s and the `.streak-ring` growth ring (appears with streak ≥1; ring fill animates ~1s after mount); the composer is COLLAPSED by default — click `.compose-collapsed` to expand it (on mobile it's inside `.compose-dock` and expands into the bottom sheet; dragging `.sheet-grip` down >25% dismisses it; desktop composer focus adds body.writing — a dimming veil), then use `.composer textarea` + `.btn.primary.small` (Save) and `.composer-close` (✕). Entries `.entry-card`: TAP (quick pointerdown+up, <400ms, <10px move) → edit mode; press-and-HOLD 400ms then drag → move to another journal (a `.entry-drag-ghost` follows the pointer, sidebar `[data-droptab]` rows are drop targets and get `.drop-hover`, dropping calls PUT /api/entries/:id with tab_id). Delete is a small `.entry-delete` ✕ (opens the in-app confirm). touch-action:pan-y on cards lets the list scroll (scrolling fires pointercancel and aborts the hold). Drive drag in tests with mouse.down → wait 500ms → stepped mouse.move to the tab → mouse.up, or synthetic PointerEvents (pointerType touch) with the same timing; deletes use the in-app `.app-dialog`, not native. Day headers `.day-header`. The "No entries yet" `.empty-state` is gated on the first `/api/entries` fetch resolving — it does NOT appear during initial load, only once loaded with zero entries. "On this day" card `.flashback` appears only when entries exist 1/3/6 months or 1–10 years back — seed with shifted `entry_date`s to test it.
...
- Photos view (nav item 3): `.gallery-thumb` grid grouped by month; clicking opens `.photo-viewer` with `.viewer-nav.prev/.next` (also ArrowLeft/Right), `.viewer-close`, touch gestures on `.viewer-stage` (horizontal swipe flips with finger-follow, swipe down closes, pinch zooms, double-tap toggles zoom — dispatch synthetic TouchEvents with rAF waits), info card with `.viewer-count`, and Open original / Delete actions (delete uses the in-app dialog).
- Nav: desktop `.desktop-nav .nav-btn` (Journal/Ask/Photos/Settings), mobile `.bottom-nav .nav-btn`. The Photos icon is a custom SVG (`.photo-icon`, camera + leaf lens, currentColor) — the others are emoji.
